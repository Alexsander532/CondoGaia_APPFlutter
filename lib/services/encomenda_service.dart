...
class EncomendaService {
  ...
  Future<List<Encomenda>> listarEncomendasPessoa({
    ...
    if ((proprietarioId == null && inquilinoId == null) ||
      ...
      throw Exception(
        'Deve ser fornecido exatamente um ID: proprietário OU inquilino',
      );
    }
    ...
    try {
      ...
      print(
        '👤 ${encomendas.length} encomendas encontradas para o $tipoPessoa',
      );
      return encomendas;
    } catch (e) {
  ...
  Future<bool> marcarComoRecebida(
    String encomendaId, {
    String? recebidoPor,
  }) async {
    try {
      ...
      print(
        '✅ Encomenda marcada como recebida: $encomendaId${recebidoPor != null ? ' por $recebidoPor' : ''}',
      );
      return true;
    } catch (e) {
  ...
  Future<bool> removerEncomenda(String encomendaId) async {
    try {
      ...
      final response = await _supabase
          .from('encomendas')
          .update({'ativo': false, 'updated_at': agora.toIso8601String()})
          .eq('id', encomendaId);
  ...
  Future<bool> excluirEncomendaPermanentemente(String encomendaId) async {
    try {
      ...
      await _supabase.from('encomendas').delete().eq('id', encomendaId);
  ...
  Future<bool> _removerFotoEncomenda(String urlFoto) async {
    try {
      ...
      // Remove do storage
      await _supabase.storage.from(_bucketFotos).remove([nomeArquivo]);
  ...
  Future<Map<String, dynamic>> verificarEstruturatabela() async {
    try {
      ...
      final count = await _supabase.from('encomendas').select('id').count();
  ...
  Future<List<Map<String, dynamic>>> listarEncomendasComNomes({
    ...
    try {
      ...
      final encomendasComNomes = (response as List<dynamic>).map((item) {
        ...
        if (item['proprietarios'] != null &&
            item['proprietarios']['nome'] != null) {
          nomeDestinatario = item['proprietarios']['nome'];
        } else if (item['inquilinos'] != null &&
            item['inquilinos']['nome'] != null) {
        ...
        // Determina quem registrou
        String registradoPor = 'Sistema';
        if (item['porteiros'] != null &&
            item['porteiros']['nome_completo'] != null) {
          registradoPor = item['porteiros']['nome_completo'];
        } else if (item['representantes'] != null &&
            item['representantes']['nome_completo'] != null) {
          registradoPor = item['representantes']['nome_completo'];
        }
        encomendaData['registrado_por'] = registradoPor;

        return encomendaData;
      }).toList();

      print(
        '📦 ${encomendasComNomes.length} encomendas com nomes encontradas para o condomínio',
      );
